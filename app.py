...
def on_start():
    ...
    if not tokens or not band_name:
        messagebox.showwarning("Warning", "토큰과 밴드를 선택해주세요.")
        return
...
# 밴드 목록 표시 라벨과 리스트 박스
tk.Label(root, text="Select Band:").grid(row=6, column=0, padx=10, pady=10, sticky="w")
band_listbox = tk.Listbox(root, height=10, width=50)
band_listbox.grid(row=7, column=0, padx=10, pady=10)

# 자동 댓글 시작 버튼
start_btn = tk.Button(root, text="Start Commenting", command=on_start)
start_btn.grid(row=8, column=0, padx=10, pady=10)
